...
object PurityStrategy extends SchedulingStrategy[Purity, Null] {
    override
    def calcPriority(dependentCell: Cell[Purity, Null], other: Cell[Purity, Null], value: Try[ValueOutcome[Purity]]): Int = value match {
        case scala.util.Success(FinalOutcome(Impure)) => -1
        case _ => 1
    }

    override
    def calcPriority(dependentCell: Cell[Purity, Null], value: Try[Purity]): Int = value match {
...
object PurityAnalysis extends ProjectAnalysisApplication {
    ...
    def doAnalyze(
        ...
        implicit
        val pool: HandlerPool[Purity, Null] = new HandlerPool(key = PurityKey, parallelism = 10, schedulingStrategy = schedulingStrategy)
