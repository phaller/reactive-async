...
abstract class AbstractIFDSAnalysis[DataFlowFact](parallelism: Int = Runtime.getRuntime.availableProcessors(), scheduling: SchedulingStrategy) extends FPCFAnalysis {
  ...
  def cont(dependee: Cell[IFDSProperty[DataFlowFact]], v: Try[ValueOutcome[IFDSProperty[DataFlowFact]]])(implicit state: State): Outcome[IFDSProperty[DataFlowFact]] = {
    handleCallUpdate(dependee.obj.asInstanceOf[(DeclaredMethod, DataFlowFact)])
  ...
  def baseMethodResult(source: (DeclaredMethod, DataFlowFact)): Outcome[IFDSProperty[DataFlowFact]] = {
    ...
    thisCell.when((c, x) ⇒ x.get, baseCell)
